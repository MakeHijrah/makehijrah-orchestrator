...
import { acceptConsultation } from "./acceptance.service.js";
...
> => {
  ...
  if (error) {
    console.error(
      "Acceptance consultant lookup failed",
      {
        profileId,
        code: error.code,
...
export const registerAcceptanceRoute = async (
  app: FastifyInstance,
): Promise<void> => {
  app.post(
    ...
    async (request, reply) => {
      ...
      const parsedParams =
        acceptanceParamsSchema.safeParse(
          request.params,
        );
