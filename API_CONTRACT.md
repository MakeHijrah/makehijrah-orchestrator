...
### Who owns which setting (Amendments 013 and 014)
...
| `standard_booking_price_cents` | **nobody** — platform configuration | consultant, admin |
| `base_consultant_commission_bps` | **nobody** — platform configuration | consultant, admin |
| `premium_consultant_commission_bps` | **nobody** — platform configuration | consultant, admin |

What is published under the platform's own domain is the platform's decision; what somebody charges for their own time, and whether they want the platform to offer them at all, is theirs. **Ownership governs writes only** — both roles read everything, and both GETs return the same object.

### Direct-booking-only (Amendment 014)

`direct_booking_only` means **"I only want direct bookings."** When true the consultant is excluded from the ordinary `/consultation` chooser — country-specific *and* general-information — and remains bookable only through their own link, at their own price, under unchanged commission rules.

The exclusion is enforced in **two** places, because they answer different questions:

- **Visibility** — the `consultants_select_active_public` RLS policy now reads `is_active = true and direct_booking_only = false`. The `/consultation` list is a direct Supabase read, so this is where the exclusion has to live. A companion policy keeps the consultant visible to a client who has **already booked them**, so their name still renders on that client's dashboard.
- **Bookability** — `POST /api/consultations/draft` refuses a standard booking naming a direct-booking-only consultant, with `error.details.reason = "consultant_direct_booking_only"`. Invisible is not the same as unbookable. **Direct bookings are unaffected.**

`direct_booking_only = true` with `direct_booking_enabled = false` is **accepted, not refused** — the consultant is then bookable nowhere, which is a state they chose.

### The calculator terms (Amendment 014)

Both GETs publish three read-only fields so a consultant's "price ↔ you earn" calculator does not hardcode percentages:

```
standard_booking_price_cents       where the premium starts
base_consultant_commission_bps     consultant share of the base portion
premium_consultant_commission_bps  consultant share above it
```

```
charged = max(price, standard_booking_price_cents)
base    = min(charged, standard_booking_price_cents)
premium = charged - base
earn    = round(base * base_bps / 10000) + round(premium * premium_bps / 10000)
```

Integer minor units, rounded **per component**, never on the blended total. **The ledger written at capture is authoritative** — this is a display estimate.

`standard_booking_price_cents` and `minimum_direct_booking_price_cents` are the **same value** (`app_settings.consultation_price_cents`) under two names answering two questions. They must never diverge.

### `PATCH /api/consultant/direct-booking`

Consultant role. Rate limit: 20 / minute. **Strict body** — exactly two keys, both optional:
...
`consultant_slug` and `direct_booking_enabled` are **absent from the schema**, so sending either is a `400`, not a silent no-op — the right answer for fields that used to work. The three calculator terms are absent too, from this schema and the admin one: they describe the platform's own commission rules and nobody edits them through a booking settings endpoint. Null clears a configured price. Consultants may still **read** their slug, enabled state and canonical booking URL from the GET above.
...
get_admin_revenue_by_source() RPC (admin only)   → /admin dashboard KPIs
get_admin_dashboard_operations() RPC (admin)     → /admin Action Required
...
**The `/admin` dashboard reads two more admin RPCs directly (migration 044).** `get_admin_revenue_by_source(current_from, current_to, compare_from, compare_to)` returns **Recorded Revenue** for both periods, grouped by currency and ledger source type; `get_admin_dashboard_operations(...)` returns booking and consultant counts, payout liability, available earnings and five alert categories in one row. Both are `authenticated`-granted, admin-gated inside, and take four explicit period bounds — the caller computes UTC month boundaries as `[monthStart, now)` against `[prevMonthStart, prevMonthStart + elapsed)`, so a month-to-date figure is never compared against a full prior month. **There is deliberately no `GET /api/admin/dashboard`**: these are direct RPC reads, matching `get_admin_finance_kpis`.

**"Recorded Revenue" is ledger-derived and is not total collected revenue.** Unattributed service purchases create no ledger entry, so their gross is excluded; `get_admin_dashboard_operations` reports that gross separately, per currency, as an alert. The UI must not label the figure "Revenue" and must not add the unattributed gross into it — until the accounting model is expanded, the two are shown side by side.
...
**Contract status: FROZEN v1.0, plus Amendments 004, 005, 006, 007, 008, 010, 011, 012, 013 and 014.** Any new endpoint requires Dave's written approval and a version bump of this document.
...
| §2c | `direct_booking_only` added to the consultant PATCH; three read-only calculator terms added to both GETs | 014 |
...
**Same-slot reselection.** If the visitor goes back and picks the time they already hold, the server **refreshes that draft's intake from the submitted request** and returns it, creating no second row, cancelling nothing and minting no second capability. Without this, the visitor would be refused by their own booking: availability counts a `draft` as busy, so slot validation would report `SLOT_TAKEN` against their own hold. The existing draft is used only when its token is bound to it, it is still `draft`, its hold has not expired, and the consultant and start time match; otherwise the request proceeds as an ordinary booking rather than reviving anything.

The refresh exists because a visitor who goes back rarely goes back only to the Time step. They may have corrected a typo in their email, fixed their name, cleared a WhatsApp number or rewritten what they want to discuss. Returning the draft untouched would discard all of it silently, and `consultation_intake.email` is not a dead snapshot — it is the address every consultation notification is actually sent to.

**Refreshed** (exactly the fields a fresh draft writes from the same body): `full_name`, `email`, `phone_whatsapp`, the `answers` object (summary, client gender, preferred consultant gender), `client_timezone`, `country_id`, and the client profile the email resolves to — the last only when the address actually changed, since it is derived from it and leaving it behind would send mail to the corrected address while dashboard access stayed under the old one.

**Never touched:** the consultation id, the consultant, the schedule, the status, `price_cents`, `currency`, `booking_source`, every Stripe identifier, every payment timestamp and everything in the ledger. None of them is a parameter of the refresh, so no caller can ask for one. Consultant eligibility (gender match and destination capability) is re-validated against the **submitted** country and preference before the refresh, so a visitor who changes their destination cannot end up holding a consultant who does not serve it.

**If the refresh fails**, the response says so — `500 INTERNAL_ERROR` for a write failure, `409 DRAFT_UNAVAILABLE` if the draft expired or was cancelled in between — and the existing draft is **not** cancelled, **no** replacement is created and its checkout capability is **not** consumed. The visitor's hold remains valid and payable. It is deliberately not reported as a slot error: the slot was never the problem, and telling somebody their time is taken when their edit failed to save sends them to fix the wrong thing.
