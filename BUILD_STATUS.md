...
- **Both layers agree on protocol-relative hrefs.** The frontend sets `allowProtocolRelative: false`; the backend refuses `//evil.example` too, reusing migration 055's `is_safe_internal_path`. An earlier draft of this section claimed the frontend allowed them — that was measured against a reconstructed policy rather than the real config, and is corrected here. **[D]**
- **service_role holds EXECUTE explicitly, not ambiently.** The BEFORE trigger calls the validator as the *invoking* role, and service_role can write `blog_posts` through Supabase's own table grants — verified against production. It also holds EXECUTE today, but only through Supabase's ambient default privileges (verified in production against `is_safe_internal_path`, which carries the identical revoke/grant pattern). Depending on that would break every orchestrator write the day the platform default changed, so the grant is stated. Migration 056's lesson, applied. **[D]**
- **Vocabulary, not serialization.** `rel`, `target` and `title` take any quote-free, angle-free text; canonicalising links to `target="_blank" rel="noopener noreferrer nofollow"` stays the frontend's job. Attribute order is not constrained. This keeps storage validation from being brittle against harmless editor changes without weakening it — none of those three attributes can execute. **[D]**
...
The approved vocabulary is the current frontend contract: `p br strong em b i u ul ol li h2 h3 a s del code pre blockquote hr`. The six formatting tags beyond the original set carry no attributes; they are typography and cannot execute.

Verification: `MIGRATION_057_VERIFICATION.sql`, **26 checks against PostgreSQL 16**, which perform real writes as `authenticated` under a blog-manager grant, as an admin, as **`service_role`**, as an ordinary user and as `anon` rather than reading the constraint and inferring — **31 legitimate editor constructs accepted** (including `<blockquote>`, `<pre><code>`, inline `<code>`, `<hr>`, `<s>`, `<del>`), **62 hostile payloads refused** (including attributes on each newly allowed tag: `<blockquote class>`, `<pre style>`, `<code onclick>`, `<hr id>`, `<s class>`, `<del data-*>`), CTA markdown source preserved, UPDATE enforced as well as INSERT, revision trigger and scheduled publishing regression-tested. Clean replay 001–057 from an empty database; 057 re-run for idempotency; the guard block proven to abort on planted unsafe rows. Production re-audited against the revised rule: **15/15 posts and 30/30 revisions pass**. Orchestrator: **840 tests pass**.
...
## 9t. Client-requested cancellation follow-up — migration 058, PROJECT_LOCK Amendment 019 **[D]**

An automatic, friendly email asking a client what happened when an admin cancels their consultation **because the client asked to**. Explicitly scoped to that one case, never to an admin's own decision or an automated path. Preceded by a full audit (this session, plan-mode) that found no client self-service cancellation endpoint exists, and no existing signal anywhere distinguishes "client asked to cancel" from any other reason an admin might cancel — the audit's central finding, and the reason this had to start with a migration rather than an email.

- **`consultations.cancellation_source`** (migration 058, authored, not applied): nullable text, `check (... in ('client_requested','admin','system'))`. Written only by `finalize_admin_consultation_cancel`, never inferred from the existing free-text `note` — the trigger is structured by construction, not by discipline. **[D]**
- **Backward compatible by construction, not by convention.** The RPC's `RETURNS TABLE` gained a column, which PostgreSQL requires a `DROP` + `CREATE` for (`CREATE OR REPLACE` refuses a return-type change) — so every grant the function already held (migrations 017, 036) had to be re-asserted **by name**, explicitly, or the drop would have silently reopened it to `anon`/`authenticated` via Supabase's ambient default privileges. Verified directly: `has_function_privilege('anon', ..., 'EXECUTE')` is `false` after the migration. An existing caller that omits the new argument resolves to `'admin'` at **two independent layers** — the orchestrator service and the RPC's own SQL default — so the two cannot disagree about what a legacy caller means. **[D]**
- **Immutable once set.** The `UPDATE` reads `coalesce(consultations.cancellation_source, v_source)`, reusing the RPC's pre-existing idempotent-replay branches. A repeat call — same consultation, different `cancellation_source` argument — reports the value the *first* successful call stored and cannot overwrite it. Proven directly against PostgreSQL, not only reasoned about: `MIGRATION_058_VERIFICATION.sql` check 8 calls the real RPC twice with different values and asserts the stored row never moved. **[D]**
- **The follow-up email is a separate file** (`client-cancellation-followup-notification.service.ts`), not a branch inside the existing admin-cancellation notification — different audience (client only), different purpose (asking what happened, not confirming an outcome), different trigger (one specific source, not every cancellation). Its own internal re-check (`status = 'cancelled' AND cancellation_source = 'client_requested'`) means a bad future call site — a test, a script, a caller that forgets the gate — still cannot make it send for the wrong reason. **[D]**
- **Recipient is `consultation_intake.email`, never `profiles.email`** — matching every other client-facing consultation notification. Tested by deliberately setting a different address on the consultant's own profile and confirming it is never the one used. **[D]**
- **The rebook link reuses, rather than reimplements, checkout's own destination logic.** `buildCancelUrl` (in `checkout.service.ts`) and the new email now both call one extracted, exported function — `buildPublicBookingDestinationUrl` (`direct-booking.slug.ts`) — so the two cannot quietly disagree about which URL a direct vs. a standard booking resolves to. The existing `checkout-cancel-url.test.ts` suite re-run unchanged after the extraction, proving it behaviour-preserving. Omitted from the email entirely, rather than a guessed URL, when a direct booking's consultant has no usable slug. **[D]**
- **Idempotent via the same Redis delivery-key pattern already used** by `admin-consultation-cancel-notification.service.ts`, `booking-notification.service.ts` and `decline-notification.service.ts` — `client-cancellation-followup:delivery:<consultationId>`, 30-day TTL. No second notification framework. **[D]**
- **Send failure never reverses or blocks the cancellation.** The RPC commits first; the email is attempted after, inside its own try/catch, logged and swallowed on failure — matching the existing admin-cancellation notification exactly. Tested by making Mandrill throw and confirming the cancellation still reports success and the row still shows `cancelled`. **[D]**
- **No historical backfill, structurally.** Every consultation cancelled before this migration reads `cancellation_source = null` permanently; the email's own eligibility check requires the literal value `'client_requested'`, so `null` can never trigger it, today or after any future schema change that might otherwise be tempted to treat `null` as a wildcard. **[D]**

**Not done, and why:** no client self-service cancellation endpoint — out of scope by explicit instruction; the audit's finding stands, and every `cancelled` consultation is still cancelled by an admin, whether acting on their own judgement or at a client's request. `cancellation_source = 'system'` is accepted by the schema but written by nothing — `abandon_draft_consultation` and `expire_stale_draft_consultations` cancel an unpaid `draft`, which migration 058 judged not to be a client-recognisable "cancelled booking," and widening those two RPCs' scope was not asked for.

Verification: `MIGRATION_058_VERIFICATION.sql`, **11 checks against PostgreSQL 16** — shape and grants read-only, then RPC behaviour against real fixtures inside a transaction that rolls back, covering backward compatibility, persistence, immutability, invalid-value refusal at both the RPC and the table `CHECK`, and refund/note behaviour left unchanged. Clean replay 001–058 from an empty database, confirmed twice. Orchestrator: **34 new tests** (20 for the notification service in isolation — eligibility, recipient, personalization, both rebook-URL branches, idempotency, Mandrill failure; 14 end to end through `adminCancelConsultation` itself, using a fixture shaped so neither Stripe nor Google Calendar needs mocking — status `confirmed`, no PaymentIntent, no calendar event, `refund: false`, which is the one combination that reaches neither branch). **874 tests pass** (up from 840). Typecheck, build and `git diff --check` clean.
