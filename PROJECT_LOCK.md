...
**Public identification fields are projections, never widened reads.** `profiles.full_name` is authoritative for a consultant's name and `profiles.avatar_url` for their photograph. Neither is publicly readable, because no public, client or anon surface may read `public.profiles`. Each therefore has a denormalised public projection on `public.consultants`: `display_name` (migration 030) and `photo_url` (migration 028). The service-role RPC `save_consultant_profile` maintains each projection **atomically** with its authoritative field, from the same argument in the same transaction — a non-null argument sets both, a null preserves both — so a projection cannot diverge from its source through the supported write path. Public booking may read `consultants.display_name` without reading `profiles`, through the pre-existing `consultants_select_active_public` policy. `profiles` RLS is **not** widened, and only these two public-safe fields are projected: not `email`, not `phone_whatsapp`, not profile metadata. Migration 030 is **authored and not applied**.
